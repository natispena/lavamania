...
class Subscription_Payu_Latam_SPL extends  WC_Payment_Subscription_Payu_Latam_SPL
{
    ...
    public function subscription_payu_latam($params)
    {
        ...
        $plan_code_description = $this->getPlanBySubscription($subscription);
        ...
        if ($id){
            ...
            $subscription->add_order_note(sprintf(__('(Subscription ID: %s)', 'subscription-payu-latam'), $id));
            ...
            $message = sprintf(__('Successful subscription (subscription ID: %s)', 'subscription-payu-latam'), $id);
    ...
    public function getProductFromSubscription(WC_Subscription $subscription)
    {
        $products =$subscription->get_items();

        $count = $subscription->get_item_count();
        if ($count > 1)
    ...
    public function paramsBilling(WC_Subscription $subscription = null)
    {
        $data = array();
    ...
    public function getPlanBySubscription(WC_Subscription $subscription)
    {
        $total_discount = $subscription->get_total_discount();
        $order_currency = $subscription->get_currency();

        $product = $this->getProductFromSubscription($subscription);
        ...
        $plan_code = $product['variation_id'] > 0 ?  "$product_name-{$product['variation_id']}" : "$product_name-$product_id";
        ...
        $plan_code = $this->cleanCharacters($plan_code);

        return array(
            "plan_description" => "Plan $plan_code",
            "plan_code" => $plan_code,
